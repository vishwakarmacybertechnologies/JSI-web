...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (shapeSelect) {
        ...
        // Helper functions for Dynamic Unit Conversion
        const getMm = (valId, unitId) => {
            const v = parseFloat(document.getElementById(valId).value) || 0;
            const u = document.getElementById(unitId).value;
            return u === 'mtr' ? v * 1000 : v; // Convert mtr to mm for area calculation
        };

        const getMtr = (valId, unitId) => {
            const v = parseFloat(document.getElementById(valId).value) || 0;
            const u = document.getElementById(unitId).value;
            return u === 'mm' ? v / 1000 : v; // Convert mm to mtr for volume calculation
        };
        ...
        calcBtn.addEventListener('click', () => {
            ...
            if (shape === 'round') {
                const d = getMm('val-diameter', 'unit-diameter');
                crossSectionalAreaMm2 = Math.PI * Math.pow((d / 2), 2);
            } else if (shape === 'flat') {
                const w = getMm('val-width', 'unit-width');
                const t = getMm('val-thickness', 'unit-thickness');
                crossSectionalAreaMm2 = w * t;
            } else if (shape === 'pipe') {
                const od = getMm('val-od', 'unit-od');
                const wt = getMm('val-wall', 'unit-wall');
                if (od > (2 * wt) && wt > 0) {
                ...
                const a = getMm('val-lega', 'unit-lega');
                const b = getMm('val-legb', 'unit-legb');
                const t = getMm('val-thickness', 'unit-thickness');
                if (t > 0 && a >= t && b >= t) {
            ...
            // Length ALWAYS converted to Meters for the final volume formula
            const lengthInMeters = getMtr('val-length', 'unit-length');

            // crossSectionalAreaMm2 * length (Meters) maps directly to volume in cm3
            const volumeCm3 = crossSectionalAreaMm2 * lengthInMeters;
        ...
        // Real-Time Auto-Calculate Bindings (Includes inputs and dropdowns)
        ...
        calculatorInputs.forEach(input => {
            ...
            input.addEventListener('change', () => calcBtn.click()); // Extra hook for selects
        });
    }
});